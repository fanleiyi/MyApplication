...
public class SimpleSongAdapter extends RecyclerView.Adapter<SimpleSongAdapter.SongViewHolder> {
    ...
    public void onBindViewHolder(SimpleSongAdapter.SongViewHolder holder, int position) {
        final Song song=mObjects.get(position);

        holder.titleTv.setText(song.getTitle());
        holder.artistTv.setText(song.getArtist());
        //在imageview上绑定一个位置
        holder.imageView.setTag(song);
        holder.imageView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Song s=(Song) v.getTag();
                //1.删除sdcard中的数据(file)
                File file=new File(s.getData());
                if(file.exists()){file.delete();}
                //2.删除数据库中的数据(存在数据库的 file 信息)
                mRecyclerView.getContext().getContentResolver()
                     .delete(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
                             MediaStore.Audio.Media._ID+"=?",
                             new String[]{String.valueOf(song.getId())});
                //3.删除内存中的数据
                mObjects.remove(song);
